...
*/
/*(function(f){var A=f.Chart,t=f.addEvent,C=f.removeEvent,k=f.createElement,n=f.discardElement,u=f.css,o=f.merge,r=f.each,p=f.extend,D=Math.max,j=document,B=window,E=f.isTouchDevice,F=f.Renderer.prototype.symbols,x=f.getOptions(),y;p(x.lang,{printChart:"打印图表",downloadPNG:"导出到EXCEL",downloadJPEG:"下载JPEG格式的图片",downloadPDF:"导出到PDF",downloadSVG:"下载SVG格式的图片",contextButtonTitle:"图表选项"});x.navigation={menuStyle:{border:"1px solid #A0A0A0",
...
(function(f) {
	...
	p(x.lang, {
		printChart : "打印图表",
		downloadPNG : "导出到EXCEL",
		contextButtonTitle : "图表选项"
	});
	x.navigation = {
		menuStyle : {
