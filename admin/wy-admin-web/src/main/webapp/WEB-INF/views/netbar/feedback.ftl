...
<body>
	...
	<table class="table table-striped table-hover">	
		...
		<#list result as res>
			<tr>
			<td>${res.id!}</td>
			<td><#if (res.img)??><img src="${imgServer!}/${res.img!}" style="width: 50px; height: 50px;" /></#if></td>
			<td>${res.create_date?string("yyyy-MM-dd HH:mm")}</td>	
			<td>
			<#if res.content?length gt 15>
			...
			<td>
			<#if res.state!=1>
			<button edit="${res.id!}" type="button" class="btn btn-info">处理</button>
			</#if>
			</td>
			</tr>
		</#list>
			...
			<div class="modal-content">
				...
				<form  class="form-horizontal form-condensed" role="form" method="post">
						...
						<div class="form-group">
							...
							<div class="col-md-10">
								<input class="form-control" type="text" name="owner_name" value="" disabled>
							</div>
						</div>
						<div class="form-group">
							<label class="col-md-2 control-label">网吧名</label>
							<div class="col-md-10">
								<input class="form-control" type="text" name="name" value="" disabled>
							</div>
						</div>
						<div class="form-group">
							<label class="col-md-2 control-label">网吧地址</label>
							<div class="col-md-10">
								<input class="form-control" type="text" name="address" value="" disabled>
							</div>
						</div>
						<div class="form-group">
							<label class="col-md-2 control-label">商户手机号码</label>
							<div class="col-md-10">
								<input class="form-control" type="text" name="owner_telephone" value="" disabled>
							</div>
						</div>
	...
	$('button[edit]').on('click', function(event) {
		...
		$.api('${ctx}/netbarFeedback/detail/' + id, {}, function(d) {
			var o = d.object;
			$("#img").attr("src",'${imgServer!}'+o.img);
			$.fillForm({
				content: o.content,
				...
				name: o.name,
				address: o.address,
				owner_telephone: o.owner_telephone,
				id: id,
			}, _$editor);
		}, function(d) {
