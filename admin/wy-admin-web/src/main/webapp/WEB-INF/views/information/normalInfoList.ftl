				...
				<div class="mb12 col-md-12">
					...
					<div class="col-md-2">
						<select class="form-control" name="isPublished">
							...
							<option value="1"<#if (params.isPublished!"0") == "1"> selected</#if>>已发布</option>
							<option value="2"<#if (params.isPublished!"0") == "2"> selected</#if>>草稿</option>
						</select>
					</div>
				</div>
				<div class="mb12 col-md-12">
			...
			<#if list??>
				<#list list as o>
					<tr>
						...
						<td>
							<a type="button" class="btn btn-info" href="${ctx}/overActivity/normal/edit?id=${(o.id)!}">编辑</a>
							...
							<#if (o.isPublished)??>
								<#if o.isPublished == 1>
									...
									<button model="${(o.id)!}" model-flag="${((o.isModel!0) == 1)?string('0', '1')}" type="button" class="btn btn-info">${((o.isModel!0) == 1)?string('取消模版', '设为模版')}</button>
								</#if>
							</#if>
						</td>
					</tr>
		...
		<script type="text/javascript">
			(function init() {
				...
				$('[model]').on('click', function(ev) {
					...
					var model = $this.attr('model-flag');
					
					operating();
					$.api('${ctx}/overActivity/model', {'id': id, 'model': model}, function(d) {
						window.location.reload();
					}, false, {
						complete: function(xhr) {
							$this.prop('disabled', false);
